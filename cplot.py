...
def read_data(code, index_code = '000001'):
    if not os.path.exists('i_data.json'):
        ...
        #for file_name in ['k_data.json', 'd_data.json', 'i_data.json']:
        #    os.remove(file_name)
        sys.exit(0)
    else:
...
def main():
    ...
    fig = plt.figure(facecolor = mcolor, figsize = (36, 36))
    mid = fig.canvas.mpl_connect("button_press_event", onclick)
    ax1 = plt.subplot2grid((6,6), (0,0), rowspan = 4 , colspan = 4, facecolor = mcolor)
    candlestick_ohlc(ax1, k_data.values, width = 0.5, colorup = 'r', colordown = 'g')

    #plot the average price
    ax1.plot(k_data.time, k_data['60price'], 'c', label = "60日成本均线", linewidth = 1.5)
    ax1.plot(k_data.time, k_data['uprice'], 'b',  label = "无穷成本均线", linewidth = 1.5)
    ...
    ax2 = plt.subplot2grid((6,6), (4, 0), rowspan = 1 , colspan = 4, facecolor = mcolor, sharex = ax1)
    ...
    # plot the index
    ax3 = plt.subplot2grid((6,6), (5, 0), rowspan = 1, colspan = 4, facecolor = mcolor, sharex = ax1)
    ax3.yaxis.set_ticks_position('left')
    ax3.xaxis.set_ticks_position('bottom')
    candlestick_ohlc(ax3, i_data.values, width = 0.5, colorup = 'r', colordown = 'g')
    ...
    ax4 = plt.subplot2grid((6,6), (0, 4), rowspan = 4, colspan = 2, facecolor = mcolor, sharey = ax1)
