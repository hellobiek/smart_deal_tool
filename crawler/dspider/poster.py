...
class Poster(object):
    ...
    def check(self):
        ...
        logger.error("{} check failed:{}, item:{}".format(self.__class__, json.dumps(errors, indent=4), self.item))
        return False
...
class PlateValuationPoster(Poster):
    ...
    def store(self):
        try:
            ...
            if insert_sql == None and params == None: return
            if not self.mysql_client.exec_sql(insert_sql, params, retry_times = 3):
                logger.error("store failed for code:{}, date:{}".format(self.item['code'], self.item['date']))
        except Exception as e:
            logger.error(e)
