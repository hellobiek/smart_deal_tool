...
def gentrends(x, window=1/3.0, charts=True):
    ...
    trends = pd.DataFrame(trends, index=np.arange(0, len(x)), columns=['Data', 'Max Line', 'Min Line'])
...
def segtrends(x, segments=2, charts=True):
    ...
    :param x: One-dimensional data set
    :param segments: How long the trendlines should be. If window < 1, then it
                   will be taken as a percentage of the size of the data
    :param charts: Boolean value saying whether to print chart to screen
    """
    y = np.array(x)
...
if __name__ == '__main__':
    ...
    end   = datetime(2018, 1, 27)
    # Download Apple price history and save adjusted close prices to numpy arra
    data = web.DataReader('TSLA', 'quandl', start, end)
    data = data.iloc[::-1]
    x = data['AdjClose']
    ...
    gentrends(x, window = 1.0/3, charts = True)

    ## Generate a series of support/resistance lines by segmenting the price history

    #segtrends(x, segments = 2, charts = True)  # equivalent to gentrends with window of 1/2
    #segtrends(x, segments = 5, charts = True)  # plots several S/R lines

    ## Generate smaller support/resistance trendlines to frame price over smaller periods
    #minitrends(x, window = 30, charts = True)

    ## Iteratively generate trading signals based on maxima/minima in given window
    #iterlines(x, window = 30, charts = True)  # buy at green dots, sell at red dots
