...
from common import get_vantage_client
...
class GetBarThread(PollingThread):
    def __init__(self, fpath, mqueue, identifiers, start_time, end_time, frequency, timezone):
        ...
        self.__subscriber  = get_vantage_client(fpath)
...
class AlphaVantageFeed(dataFramefeed.TickFeed):
    """
        a real-time BarFeed that builds bars using alpha vantage api
        :param identifiers: codes
        :param frequency 每隔几秒钟请求一次，默认3秒钟
        :param maxLen:
    """
    def __init__(self, identifiers, timezone, dealtime, fpath, frequency = 3, maxLen = DEFAULT_MAX_LEN):
        ...
        self.logger = getLogger(__name__)
        self.__thread = GetBarThread(fpath, self.__queue, identifiers, self.__start_time, self.__end_time, timedelta(seconds = frequency), self.__timezone)
