...
class Broker(broker.Broker):
    ...
    def __init__(self, cash, barFeed, market = ct.CN_MARKET_SYMBOL, commission = None):
    ...
    def updateNominalPrice(self, bars):
        for instrument, shares in self.__shares.items():
            price = self._getPriceForInstrument(instrument)
            cprice = self.position.loc[self.position.code == instrument]['cost_price']
            self.position.loc[self.position.code == instrument, 'nominal_price'] = price
            self.position.loc[self.position.code == instrument, 'pl_ratio'] = 100 * (price - cprice) / cprice

    def setPositionAndShares(self, order, shares_delta, price):
        instrument = order.getInstrument()
        updated_shares = order.getInstrumentTraits().roundQuantity(self.getShares(instrument) + shares_delta)
        if self.position is None:
            mdict = {'code': instrument, 'qty': updated_shares, 'nominal_price': price, 'cost_price': price, 'pl_ratio': 0}
            data = pd.DataFrame([mdict])
            self.position = data
            self.__shares[instrument] = updated_shares
        else:
            current_qty = self.position.loc[self.position.code == instrument]['qty']
            cost_price = self.position.loc[self.position.code == instrument]['cost_price']
            if instrument in self.position.code.tolist():
                if updated_shares == 0:
                    self.position = self.position[self.position.code != instrument]
                    self.position = self.position.reset_index(drop = True)
                    if self.position.empty: self.position = None
                    del self.__shares[instrument]
                else:
                    cost_price = (cost_price * current_qty + shares_delta * price) / updated_shares
                    pl_ratio = 100 * (price - cost_price) / cost_price
                    self.position.loc[self.position.code == instrument] = [instrument, updated_shares, price, cost_price, pl_ratio]
                    self.__shares[instrument] = updated_shares
            else:
                mdict = {'code': instrument, 'qty': updated_shares, 'nominal_price': price, 'cost_price': price, 'pl_ratio': 0}
                data = pd.DataFrame([mdict])
                self.position = self.position.append(data)
                self.__shares[instrument] = updated_shares
    ...
    def getPositions(self):
        ...
        df = self.position.copy()
        df['code'] = df.apply(lambda row: add_prifix(row['code']), axis = 1)
        return df
    ...
    def _getPriceForInstrument(self, instrument):
        ...
        if lastBar is not None:
            ...
            raise Exception("get price failed for {}".format(instrument))
        return ret
    ...
    def commitOrderExecution(self, order, dateTime, fillInfo):
        ...
        if resultingCash >= 0 or self.__allowNegativeCash:
            ...
            self.setPositionAndShares(order, sharesDelta, price)
    ...
    def submitOrder(self, order, sname):
        if order.isInitial():
