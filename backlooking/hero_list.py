...
def get_hero_list_info(mdate, data_dir):
    ...
    df = pd.read_csv(fullfilepath, sep = ',', dtype = {'date':str, 'code':str, 'name': str, 'pchange': float, 'type': str, 'buy': int, 'sell': int})
...
def generate_hero(mdate, data_dir, dirname):
    ...
    t_index = MarkdownTable(headers = ["代码", "名称", "类型", "涨幅", "买入额", "卖出额", "净额"])
    for index in range(len(hero_info_one)):
        data_list =  hero_info_one.loc[index].tolist()
        content_list = [data_list[1], data_list[2], data_list[3], data_list[4], data_list[5], data_list[6], data_list[7]]
