...
def compute_Hc(series, kind = "random_walk", min_window = 10, max_window = None, simplified = True):
    ...
    """
    if len(series)<40: raise ValueError("Series length must be greater or equal to 40")
    ...
    if type(series) not in ndarray_likes: series = np.array(series)

    if "pandas.core.series" in sys.modules.keys() and type(series) == pd.core.series.Series:
        if series.isnull().values.any(): raise ValueError("Series contains NaNs")
...
if __name__ == '__main__':
    ...
    np.random.seed(100)
    series = np.random.random(60) * 100
