...
class Groupby:
    def __init__(self, keys):
        """
        :param keys: List of group identifiers. Both __init__ and apply will run much faster if keys is already sorted.
        """
        try:
        ...
        if already_sorted:
            ...
            if keys.ndim > 1: raise ValueError('keys should be 1-dimensional')
            ...
            _, self.first_occurrences, self.keys_as_int = np.unique(keys, return_index=True, return_inverse=True)
            self.n_keys = max(self.keys_as_int) + 1
        self.indices = self._set_indices()

    def _set_indices(self):
        if self.already_sorted:
            indices = [slice(i, j) for i, j in zip(self.first_occurrences[:-1], self.first_occurrences[1:])]
            assert isinstance(indices, list)
            indices.append(slice(self.first_occurrences[-1], len(self.keys_as_int)))
            indices = np.array(indices)
        else:
            indices = [[] for _ in range(self.n_keys)]
            for i, k in enumerate(self.keys_as_int): indices[k].append(i)
    ...
    def apply(self, function_, array, broadcast=True, shape=None, order='c'):
        ...
        if broadcast:
            ...
            if self.already_sorted:
                ...
                for k, idx in enumerate(self.indices):
                    result[self.keys_as_int[self.first_occurrences[k]]] = function_(np.take(array, idx, 0))
        return result
