...
from base.cdate import quarter, transfer_date_string_to_int, report_date_with, str_to_datetime, int_to_datetime, prev_report_date_with
class CValuation(object):
    ...
    def convert(self, mdate = None):
        #date, code, 1.基本每股收益、2.扣非每股收益、4.每股净资产、6.净资产收益率(roe)、
        #72.所有者权益（或股东权益）合计、96.归属于母公司所有者的净利润、238.总股本、239.已上市流通A股、
        #242.股东人数、243.第一大股东的持股数量、
        #248.QFII机构数、249.QFII持股量、250.券商机构数、251.券商持股量、
        ...
        #35.商誉、财报披露时间
        mcols = ['date','code','eps','neps','bps','roe','na','np','tcs','ccs',
                 'holders','largest_holding','qfii_holders','qfii_holding',
                 'broker_holders','broker_holding','insurance_holders','insurance_holding',
                 'fund_holders','fund_holding','social_security_holders','social_security_holding',
                 'private_holders','private_holding','financial_company_holders', 'financial_company_holding',
                 'annuity_holders','annuity_holding','goodwill','publish']
        ...
        for mdate in date_list:
            ...
            for idx, row in report_df.iterrows():
                ...
                report_list.append([row['date'], row['code'], row[prefix%1], row[prefix%2], row[prefix%4], 
                            ...
                            row[prefix%242], row[prefix%243], row[prefix%248], row[prefix%249], 
                            row[prefix%250], row[prefix%251], row[prefix%252], row[prefix%253], 
                            row[prefix%254], row[prefix%255], row[prefix%256], row[prefix%257], 
                            row[prefix%258], row[prefix%259], row[prefix%260], row[prefix%261], 
                            row[prefix%262], row[prefix%263], row[prefix%35], pdate])
